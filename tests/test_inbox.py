...
class InboxTest(unittest.TestCase):
    ...
    def test_write_and_load_note(self):
        inbox.write_note(self.target, "picard-3", "A.foo", "generalize it", "had to re-prove")
        boxes = inbox.load_all(self.target)
        self.assertEqual(len(boxes), 1)
        self.assertEqual(boxes[0].author, "picard-3")
        self.assertEqual(boxes[0].notes[0].lean_name, "A.foo")
        self.assertEqual(boxes[0].notes[0].status, "open")
        self.assertEqual(boxes[0].notes[0].rationale, "had to re-prove")

    def test_one_file_per_author_update_in_place(self):
        inbox.write_note(self.target, "picard-3", "A.foo", "v1")
        inbox.write_note(self.target, "picard-3", "A.foo", "v2 better")
        ...
        self.assertEqual(boxes[0].notes[0].suggestion, "v2 better")

    def test_distinct_decls_append(self):
        inbox.write_note(self.target, "picard-3", "A.foo", "x")
        inbox.write_note(self.target, "picard-3", "A.bar", "y")
    ...
    def test_distinct_authors_distinct_files(self):
        inbox.write_note(self.target, "picard-3", "A.foo", "x")
        inbox.write_note(self.target, "ramanujan", "A.foo", "y")
    ...
    def test_renote_resets_status_to_open(self):
        ...
        inbox.write_note(self.target, "picard-3", "A.foo", "reconsider please")
        self.assertEqual(len(inbox.open_notes(self.target)), 1)

    def test_author_filename_sanitized(self):
        inbox.write_note(self.target, "parent/child", "A.foo", "x")
