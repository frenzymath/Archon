...
Every block you write that derives from external reference material needs: a `% SOURCE:` comment (`<pointer> (read from references/<file>.md)`), a `% SOURCE QUOTE:` with the **verbatim** original-language statement copied from that local file, a `% SOURCE QUOTE PROOF:` before the proof environment when applicable, and a visible `\textit{Source: <pointer>.}` first line. **Never write any of these from training memory.** If you lack the local source, spawn a `reference-retriever` (Bash, blocking — needs `references/**` in your write-domain), wait for it, open the new `references/<slug>.md`, then write the citation block. If retrieval fails, leave the block flagged and report it INCOMPLETE — never substitute a paraphrase or a remembered quote.
