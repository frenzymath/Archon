...
10. Optionally invoke subagents (see "Subagent delegation" below). Mandatory ones in your catalog are tagged `[MANDATORY]` — you MUST dispatch them this phase. Use `strategy-auditor` to validate strategic routes against reference PDFs, and `blueprint-clean` to enforce blueprint purity. Use `lean-scaffolder` to set up Lean files and convey implementation hints.
...
- **Medium content** (a paragraph or two): delegate to the `lean-scaffolder` subagent to inject these into the corresponding `.lean` file as a `/- Blueprint note: ... -/` or `/- Planner strategy: ... -/` block above the target declaration. You MUST NOT edit `.lean` files yourself.
