...
class InnerGit:
    ...
    _DEFAULT_EXCLUDES = (
        ...
        "__pycache__/\n"
        ".git/\n"
        "# The inner repo's OWN git directory — it must never track itself.\n"
        "# `_stage_all` skips `git-dir` when force-adding `.archon/` children,\n"
        "# but the plain `git add -A` it runs first would otherwise stage it\n"
        "# whenever the outer `.gitignore` lacks an `.archon/` rule (e.g. a\n"
        "# project with no outer git repo, or a pre-rule upgrade). Tracking it\n"
        "# leaves every commit perpetually dirty (the index/refs/objects churn\n"
