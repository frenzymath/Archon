...
function getBaseUrl(): string {
  ...
  if (base === './' || base === '.') {
    ...
    // A trailing slash means we're already sitting at the app's mount
    // directory. Otherwise the final segment is NOT part of the base — it is
    // either a static filename (index.html) or a client-side route the SPA
    // router owns (e.g. /dag, /blueprint), which is exactly the deep-link /
    // hard-refresh case. In both cases strip it back to the directory. (A
    // dotted heuristic misfired here: a dotless route like /dag was treated
    // as a directory and produced /dag/ as the base, breaking API/WS URLs and
    // the router basename on refresh.)
    if (!loc.endsWith('/')) {
